...
class report_account_aged_partner(models.AbstractModel):
	...
	def _get_lines(self, options, line_id=None):
		...
		#####################################################################################################
		# Se consulta si el reporte es de tipo Vencidas por pagar/cobrar en USD, sino continua con su
		# ejecucion original. Si ha de serlo se llena la variable user_currency con el id de la moneda del
		# Dolar e imita el comportamiento original convirtiendo toda factura a USD.
		#####################################################################################################
		account_types = [self.env.context.get('account_type')]
		if not account_types in [['payable_usd'],['receivable_usd']]:
			return super(report_account_aged_partner, self)._get_lines(options, line_id)
		user_currency = self.env['res.currency'].search([('name','=','USD'),('id','=',2),('symbol','=','$')])
		#####################################################################################################
		...
		for values in results:
			vals = {
				...
				'name': values['name'],
				'level': 2,
				'columns': [{'name': ''}] * 4 + [{'name': self.format_value(sign * v, currency=user_currency), 'no_format': sign * v} for v in [ values['direction'], values['4'], values['3'], values['2'], values['1'], values['0'], values['total']]],
				'trust': values['trust'],
				'unfoldable': True,
				...
				for line in amls[values['partner_id']]:
					...
					vals = {
						...
						'columns': [{'name': v} for v in [format_date(self.env, aml.date_maturity or aml.date), aml.journal_id.code, aml.account_id.display_name, format_date(self.env, aml.expected_pay_date)]] +
								   [{'name': self.format_value(sign * v, currency=user_currency, blank_if_zero=True), 'no_format': sign * v} for v in [line['period'] == 6-i and line['amount'] or 0 for i in range(7)]],
						'action_context': {
		...
		if total and not line_id:
			total_line = {
				...
				'class': 'total',
				'level': 2,
				'columns': [{'name': ''}] * 4 + [{'name': self.format_value(sign * v, currency=user_currency), 'no_format': sign * v} for v in [total[6], total[4], total[3], total[2], total[1], total[0], total[5]]],
			}
...
class ReportAgedPartnerBalance(models.AbstractModel):
	...
	def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
		...
		# Dolar e imita el comportamiento original convirtiendo toda factura a USD.
		#####################################################################################################
		if not account_type in [['receivable_usd'],['payable_usd']]:
			return super(ReportAgedPartnerBalance, self)._get_partner_move_lines(account_type, date_from, target_move, period_length)
		#####################################################################################################
		account_type = ['receivable'] if account_type == ['receivable_usd'] else ['payable']
		user_currency = self.env['res.currency'].search([('name','=','USD'),('id','=',2),('symbol','=','$')])
		#####################################################################################################
		...
		for i in range(5):
			...
			for line in move_lines:
				...
				line_amount = line.amount_currency if line.currency_id == user_currency else line.company_id.currency_id._convert(line.balance, user_currency, user_company, line.move_id.date, round = False)
				if user_currency.is_zero(line_amount):
					continue
				for partial_line in line.matched_debit_ids:
					if partial_line.max_date <= date_from:
						line_amount += partial_line.amount_currency if partial_line.currency_id == user_currency else partial_line.company_id.currency_id._convert(partial_line.amount, user_currency, user_company, partial_line.create_date, round = False)
...
class report_account_aged_receivable(models.AbstractModel):
	...
	def _set_context(self, options):
		...
		ctx['account_type'] = 'receivable' if not 'USD' in ctx else 'receivable_usd'
		return ctx
...
class report_account_aged_payable(models.AbstractModel):
	...
	def _set_context(self, options):
		...
		ctx['account_type'] = 'payable' if not 'USD' in ctx else 'payable_usd'
