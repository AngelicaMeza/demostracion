...
class IslrWhDoc(models.Model):
 ...
 #       vals[0]['code'] = code
    ...
    def action_move_create(self):
        ...
        """
        #context = self._context or {}
        # print("\n self en el def action_move_create   ", self, "\n\n")
        # MODELO islr.wh.doc
        ids = isinstance(self.ids, (int)) and [self.ids] or self.ids   # ¿Para qué pregunta si es un entero?
        # print("\n  isinstance(self.ids, (int))  ", isinstance(self.ids, (int)), "\n")
        # print("\n   TYPE isinstance(self.ids, (int))  ", type(isinstance(self.ids, (int))), "\n")
        # print("\n  [self.ids] ", self.ids, "\n")
        # print("\n ids   ", ids, "\n")
        ...
        ret = self.browse(ids)
        #ret islr.wh.doc ( registro específico,  )
        # print("\n  ret  ", ret, "\n")
        self = self.with_context({'income_wh': True})
        # self islr.wh.doc ( en cierto registro ,)
        # print("\n  self.with_context({'income_wh': True})  ", self.with_context({'income_wh': True}), "")
        ...
        if not ret.date_uid:
            self.write({'date_uid': time.strftime('%Y-%m-%d')})

        ret.refresh()
        if ret.type in ('in_invoice', 'in_refund'):
            self.write({'date_ret': ret.date_uid})
        else:
            if not ret.date_ret:
        ...
        # print("\n ret.invoice_ids  ", ret.invoice_ids, "\n")
        for line in ret.invoice_ids:
            # print("\n line  ", line, "\n")
            # print("\n ret.name ", ret.name, "\n")
            if ret.type in ('in_invoice', 'in_refund'):
                #print("type(ret.name)", print(type(ret.name)), "\n")
                # print("type(line.invoice_id.supplier_invoice_number )", print(type(line.invoice_id.supplier_invoice_number)), "\n")
                # print("ret.name", print(ret.name), "\n")
                # print("line.invoice_id.supplier_invoice_number ", print(line.invoice_id.supplier_invoice_number ), "\n")
                name = 'COMP. RET. ISLR ' + ret.name + ' Doc. ' + (line.invoice_id.supplier_invoice_number or '')
            else:
                # print("type(ret.number)", print(type(ret.number)), "\n")
                # print("type(line.invoice_id.display_name )", print(type(line.invoice_id.display_name )), "\n")
                # print("ret.number", print(ret.number), "\n")
                # print("line.invoice_id.display_name", print(line.invoice_id.display_name), "\n")
                name = 'COMP. RET. ISLR ' + ret.name + ' Doc. ' + (line.invoice_id.display_name or '')
                # print("name ",name , "\n")
