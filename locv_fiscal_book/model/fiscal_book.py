...
class FiscalBook(models.Model):
   ...
   # company_id = fields.Many2one('res.company', 'Compañia', help='Company', )
        ...
        for fb_brw in local_fb:
            ...
            for lin in self.fbl_ids:
                ...
                for lin2 in self.fbl_ids:
                    if lin.doc_type == lin2.doc_type and lin.emission_date == lin2.emission_date and lin.ctrl_number == lin2.ctrl_number and lin.invoice_number == lin2.invoice_number and lin.wh_number == lin2.wh_number and lin.iwdl_id == lin2.iwdl_id:
                        condi += 1
                if condi > 1:
                    lin.unlink()
    ...
    def update_book_issue_invoices(self, fb_id):
        ...
        for invoice in issue_inv_ids:
            ...
            if invoice.type == 'in_invoice':
                ...
                #     docum = invoice.debit_origin_id.name
                # else:
                docum = invoice.name
            elif invoice.type == 'in_refund' or 'out_refund':
                docum = invoice.invoice_origin    #FALTA
    ...
    def update_book_lines(self, fb_id):
        ...
        if not self.browse(fb_id).issue_invoice_ids and self.iwdl_ids:
            for inv_brw in self.iwdl_ids:
                if inv_brw.state == 'done':
                    inv_iva += inv_brw        
        if inv_iva:
