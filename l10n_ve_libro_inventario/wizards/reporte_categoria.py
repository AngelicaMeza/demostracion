...
class ReporteCategoria(models.TransientModel):
            ...
            for item in self.product:
                ...
                if len(kardex_line) > 0:
                    ...
                    for sal in kardex_line :
                        if sal.stock_move_id == '10':
                            libro.cantidad_ajuste += sal.cantidad_entradas - sal.cantidad_salidas 
                            libro.total_bolivares_ajuste += sal.total_bolivares_entradas - sal.total_bolivares_salida 
                        else:
                    ...
                    libro.costo_ajuste = libro.total_bolivares_ajuste / libro.cantidad_ajuste if libro.total_bolivares_ajuste > 0 else 0 
    ...
    def print_facturas(self):
        self.datos()
        return self.env.ref('l10n_ve_libro_inventario.movimientos_categoria_libro').report_action(self)
...
class LibroVentasModelo(models.Model):
    ...
    cantidad_ajuste  = fields.Float(string='Cantidad ajuste')
    costo_ajuste     = fields.Float(string='Costo de ajuste')
    total_bolivares_ajuste  = fields.Float(string='Total Bolivares')
