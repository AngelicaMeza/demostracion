...
class AccountWhIvaLineTax(models.Model):
    ...
    def _get_VES_rate_from_invoice(self, invoice_id):
        ...
        return self.env['res.currency']._get_conversion_rate(USD, VES, company, date)
    ...
    def _get_amount_ret(self):
        ...
        for record in self:
            ...
            # WITHHOLDING
            print(record.wh_vat_line_id.wh_iva_rate )
...
class AccountWhIva(models.Model):
    ...
    def _get_valid_wh(self, amount_ret, amount, wh_iva_rate,
        ...
        if amount != 0 and amount_ret != 0:
            amount_total = '{0:,.4f}'.format(amount * (wh_iva_rate - offset) / 100.0)
            amount_total2 = '{0:,.4f}'.format(amount * (wh_iva_rate + offset) / 100.0)
        else:
            amount_total = 0
            amount_total2 = 0
        ...
        for ret in self.with_context(ctx):
            ...
            if ret.wh_lines:
                ...
                for line in ret.wh_lines:
                    ...
                    date = ret.wh_lines.invoice_id.date
                    name = name
                    ...
                    if (line.invoice_id.currency_id.id != line.invoice_id.company_id.currency_id.id):
                        # f_xc = self.env['res.currency']._get_conversion_rate(line.invoice_id.currency_id.name, line.invoice_id.company_id.currency_id.name, invoice.company_id, invoice.date)
                        
                        for ml in self.env['account.move.line'].search([('move_id', '=', ret_move.id)]):
                            ml.write({
                                'currency_id': line.invoice_id.currency_id.id})

                            if ml.credit and ml.amount_currency_dollar:
                              
                                ml.write({
                                    'amount_currency':  ml.amount_currency_dollar})

                            elif ml.debit and ml.amount_currency_dollar:
                                ml.write({
                                    'amount_currency':  ml.amount_currency_dollar})
                    # for lala in ret_move:
                    #     for lala in lala.invoice_line_ids:
                    #         print('aja')
                    ret_move.date = ret.date_ret
                    ret_move.post()
